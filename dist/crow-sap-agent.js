...
exports.CrowSapAgent = void 0;
const index_js_1 = require("../synapse-sap-sdk/dist/cjs/index.js");
...
const DEFAULT_CONFIG = {
    ...
    agentDescription: "Autonomous data analytics agent powered by Ace Data Cloud. " +
        "Queries Solana RPC via x402 pay-per-use and stores memory in SAP vault.",
    aceDataCloudEndpoint: process.env.ACE_DATA_CLOUD_ENDPOINT ||
...
class CrowSapAgent {
    ...
    constructor(config = {}) {
        ...
        if (this.config.keypairPath && fs.existsSync(this.config.keypairPath)) {
            ...
            this.keypair = web3_js_1.Keypair.fromSecretKey(Uint8Array.from(keyData));
        }
        else {
    ...
    async initialize() {
        ...
        try {
            this.client = (0, index_js_1.createSapClient)(this.config.rpcUrl);
    ...
    async register() {
        ...
        const pricing = [
            { tier_id: "standard", price_per_call: new anchor_1.BN(this.config.pricePerCall), min_price_per_call: null, max_price_per_call: null, rate_limit: 60, max_calls_per_session: 1000, burst_limit: null, token_type: {}, token_mint: null, token_decimals: null, settlement_mode: null, min_escrow_deposit: null, batch_interval_sec: null, volume_curve: null },
        ];
        if (this.client) {
            try {
                // Derive PDAs
                const [agentPDA] = index_js_1.Pdas.getAgentPDA(this.keypair.publicKey);
                const [agentStatsPDA] = index_js_1.Pdas.getAgentStatsPDA(this.keypair.publicKey);
                const [globalPDA] = index_js_1.Pdas.getGlobalPDA();
    ...
    async openEscrow() {
        ...
        if (this.client) {
            try {
                ...
                if (aceWalletStr) {
                    const aceWallet = new web3_js_1.PublicKey(aceWalletStr);
                    const [escrowPDA] = index_js_1.Pdas.getEscrowV2PDA(aceWallet, 0);
                    const [agentPDA] = index_js_1.Pdas.getAgentPDA(aceWallet);
                    const [agentStakePDA] = index_js_1.Pdas.getAgentStakePDA(aceWallet);
                    const [agentStatsPDA] = index_js_1.Pdas.getAgentStatsPDA(aceWallet);
                    const [pricingPDA] = index_js_1.Pdas.getAgentPDA(aceWallet); // simplified
                    const ix = await this.client.escrow.createEscrowV2({
                        ...
                        escrow: escrowPDA,
                        escrowNonce: new anchor_1.BN(0),
                        pricePerCall: new anchor_1.BN(this.config.pricePerCall),
                        maxCalls: new anchor_1.BN(100),
                        initialDeposit: new anchor_1.BN(this.config.maxEscrowDeposit),
                        expiresAt: new anchor_1.BN(0),
                        volumeCurve: [],
                        tokenMint: null,
                        tokenDecimals: 9,
                        settlementSecurity: 0,
                        disputeWindowSlots: new anchor_1.BN(0),
                        coSigner: null,
                        arbiter: null,
                    });
                    const tx = await this.client.buildTransaction([ix], this.keypair.publicKey);
    ...
    async queryBalance(address) {
        ...
        let balance = 0;
        try {
            const response = await fetch(`${this.config.aceDataCloudEndpoint}/rpc/getBalance`, {
                method: "POST",
            ...
            if (response.status === 402) {
                ...
                console.log(`[Crow] Balance: ${balance / web3_js_1.LAMPORTS_PER_SOL} SOL`);
            }
        }
        catch (err) {
            console.log(`[Crow] RPC call error (expected in demo): ${err.message}`);
            balance = Math.floor(Math.random() * 10 * web3_js_1.LAMPORTS_PER_SOL);
            console.log(`[Crow] Simulated balance: ${balance / web3_js_1.LAMPORTS_PER_SOL} SOL`);
        }
        this.remember(`Queried balance for ${address}: ${balance / web3_js_1.LAMPORTS_PER_SOL} SOL`);
        return balance;
    }
    async queryAccountInfo(address) {
        console.log(`[Crow] Step 4b: Querying account info for ${address}...`);
        this.state.callsMade++;
        this.state.totalSpent += this.config.pricePerCall;
        let accountInfo = null;
        try {
            const response = await fetch(`${this.config.aceDataCloudEndpoint}/rpc/getAccountInfo`, {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({
                    jsonrpc: "2.0",
        ...
        if (this.client) {
            try {
                ...
                if (aceWalletStr) {
                    ...
                    const [agentStatsPDA] = index_js_1.Pdas.getAgentStatsPDA(aceWallet);
                    const [escrowPDA] = index_js_1.Pdas.getEscrowV2PDA(aceWallet, 0);
                    const [settlementReceipt] = index_js_1.Pdas.getPendingSettlementPDA(escrowPDA, 0);
                    const ix = await this.client.escrow.settleCallsV2({
                        ...
                        settlementReceipt,
                        escrowNonce: new anchor_1.BN(0),
                        callsToSettle: new anchor_1.BN(this.state.callsMade),
                ...
                console.error(`[Crow] Settlement failed: ${err.message}`);
            }
        }
        else {
            console.log(`[Crow] Simulated settlement: ${this.state.callsMade} calls, ` +
                `${this.state.totalSpent} lamports`);
        }
        this.remember(`Settled payments: ${this.state.callsMade} calls, ${this.state.totalSpent} lamports`);
    }
    ...
    async storeMemory() {
        ...
        if (this.client) {
            try {
                const [agentPDA] = index_js_1.Pdas.getAgentPDA(this.keypair.publicKey);
                const [vaultPDA] = index_js_1.Pdas.getVaultPDA(agentPDA);
                const [sessionPDA] = index_js_1.Pdas.getSessionLedgerPDA(vaultPDA, 1);
                // Open session
                ...
                const [checkpointPDA] = index_js_1.Pdas.getEpochPagePDA(vaultPDA, 1);
                ...
                console.error(`[Crow] Vault write failed: ${err.message}`);
            }
        }
        else {
    ...
    generateReport() {
        return {
            ...
            totalSpentSol: this.state.totalSpent / web3_js_1.LAMPORTS_PER_SOL,
    ...
    // ─── Full Workflow ──────────────────────────────────────────────────────
    async runFullWorkflow(targetAddresses = []) {
...
exports.CrowSapAgent = CrowSapAgent;
// ─── Main ────────────────────────────────────────────────────────────────────
async function main() {
    ...
    const reportPath = path.join(reportDir, `crow-sap-report-${new Date().toISOString().replace(/[:.]/g, "-")}.json`);
...
//# sourceMappingURL=crow-sap-agent.js.map